function pageRefreshed() {
	function createButtons(someDiv) {
		let el = document.createElement('script');
		  el.src = chrome.runtime.getURL('assets/js/jquery.js');
		...
		if($('.player-controls__right-control-group').length){ 
			if(!$('.clean').length){ 
				...
				$(".cleanButton").css({"background-image": "url("+chrome.runtime.getURL("assets/img/clean-icon.png")+")"});
			...
			if(!$('.start').length){
				...
				$(".startButton").css({"background-image": "url("+chrome.runtime.getURL("assets/img/start-icon.png")+")"});
...
function start() {
	...
	if($(".startButton").data('state')=="stopped"){
		...
		$(".startButton").css({"background-image": "url("+chrome.runtime.getURL("assets/img/stop-icon.png")+")"});
  ...
  if (request.method == 'blobs'){
	...
	$(".size").removeClass('notransition');
	  ...
	  if(!$(".download").length){
		...
		$(".downloadButton").css({"background-image": "url("+chrome.runtime.getURL("assets/img/download-icon.png")+")"});
